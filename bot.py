...
def text(message: types.Message):
    if message.chat.type == 'private':
        if message.text.lower() == 'продукты':
            category = get_category('http://164.90.224.228/product/category/')      
            markup = create_inline_markup(row_width=3, kwargs=category)
            bot.send_message(message.chat.id, 'Выберите категорию:', reply_markup=markup)
        if message.text.lower() == 'корзина':
            try:
                for i in requests.get('http://164.90.224.228/user/user/').json():
            ...
            for i in requests.get('http://164.90.224.228/user/cart/').json():
                if str(i['user']) == str(user_id):
                    product = i
                    get_product_image('http://164.90.224.228/product/products')
                    photo = product['image']
                    photo = open(photo, 'rb')
...
def callback_handler(call):
    try:
        if call.message:
            if call.data in [str(i['id']) for i in requests.get('http://164.90.224.228/product/category/').json()]:
                category = get_category('http://164.90.224.228/product/category/')
                ...
                quantity = 0
                for i in requests.get('http://164.90.224.228/product/products/').json():
            ...
            if call.data in [str(i['call_back']) for i in requests.get('http://164.90.224.228/product/products/').json()]:
                for i in requests.get('http://164.90.224.228/product/products/').json():
